...
const { cloneDeep, merge, has: hasKey } = require('lodash');
...
let generateRandomModel = async (model, opts) => {
  ...
  for (let field in paths) {
    ...
    if (opts.ignore && opts.ignore.indexOf(field) !== -1) {
    ...
    if (desc.isEnum) {
      ...
      if (type === 'string') {
        if (opts.autoDetect) {
          /* Attempt to detect e-mail, password, or phone */
          if (/e?\-?mail/im.test(field)) {
            generated[field] = faker.internet.email();
          } else if (/password/im.test(field)) {
            generated[field] =
              faker.name.firstName() +
              faker.name.lastName() +
              Math.random() +
              '_';
          } else if (/phone/im.test(field)) {
            generated[field] = faker.phone.phone_number.phoneNumber();
          } else {
        ...
        for (let i = 0; i < faker.random.number(15); i++) {
          ...
          if (arrayDef.isPathDef) {
            generated[field].push(
              await generateRandomModel(
                { generate: desc.arrayDefinition },
                opts
              ).generate
            ); /* Handle arrays with primitives */
          } else {
            generated[field].push(
              await generateRandomModel(desc.arrayDefinition, opts)
